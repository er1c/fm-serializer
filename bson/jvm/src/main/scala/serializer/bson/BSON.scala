...
import org.bson.{BsonBinaryWriter, BsonDocument, BsonDocumentReader, BsonDocumentWriter, BsonReader, BsonWriter, RawBsonDocument}
...
object BSON {
  ...
  def fromBsonBytes[T](bytes: Array[Byte])(implicit deserializer: Deserializer[T]): T = {
    fromBsonDocument[T](new RawBsonDocument(bytes))
  }

  def fromBsonDocument[T](doc: BsonDocument)(implicit deserializer: Deserializer[T]): T = {
    fromReader(new BsonDocumentReader(doc))(deserializer)
  }
