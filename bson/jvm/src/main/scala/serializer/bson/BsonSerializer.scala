...
object UUIDSerializer {
  implicit class RichUUID(val uuid: UUID) extends AnyVal {
...
final class UUIDSerializer extends BsonSerializer[UUID] {
  ...
  protected val normal: SimpleSerializer[UUID] = Primitive.string.map[UUID](defaultValue){ id: UUID => id.toString }{ hex: String => UUID.fromString(hex) }
  ...
  private def fromBsonBinary(b: BsonBinary): UUID = UUID.nameUUIDFromBytes(b.getData())
}
...
object BsonSerializer extends SerializerObj with BsonImplicits {
