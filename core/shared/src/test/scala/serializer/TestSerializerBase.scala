...
import serializer.validation.{Validation, ValidationError, ValidationResult}

object TestSerializerBase {
  ...
  case class MostlyEmptyFoo(@Field(18 /* Make sure this is synced up with Foo */) bar: Bar)
...
trait TestSerializerBase[BYTES] extends FunSuite with Matchers with AppendedClues {
  import TestSerializerBase._
  ...
  private def checkPrimitive[@specialized T](value: T)(implicit ser: Serializer[T], deser: Deserializer[T]): Unit = {
    ...
    val newValue = deserialize[T](bytes)
    withClue(s"original $value, serialized: '$bytes', ${bytes.getClass} new: $newValue") { newValue shouldBe value }
  }
  ...
  case class NonCompressedNums(
    @Field(1, serializer.Primitive.fixedInt) fixedInt: Int,
    @Field(2, serializer.Primitive.fixedLong) fixedLong: Long
  )
