...
abstract class MacroHelpers(isDebug: Boolean) { self =>
  ...
  private case class ObjectSerializationInfo(objTpe: Type, fields: Vector[FieldImpl]) {
    ...
    def serializerDeclarations: Vector[ValDef] = sharedSerializers.filterNot{ case (_,name) => name == termNames.EMPTY }.map{ case (tpe, name) =>
      // Use a more specific type than serializer.Serializer[$tpe] if we know it.
      ...
      getImplicit(appliedType(typeOf[Serializer[_]], List(tpe)), withMacrosDisabled = true) match {
        ...
        case None       => q"private[this] lazy val $name: serializer.Serializer[$tpe] = implicitly[serializer.Serializer[$tpe]]"
      }
  ...
  private case class ObjectDeserializationInfo(objTpe: Type, fields: Vector[FieldImpl]) {
    ...
    def deserializerDeclarations: Vector[ValDef] = sharedDeserializers.filterNot{ case (_,name) => name == termNames.EMPTY }.map{ case (tpe, name) =>
      getImplicit(appliedType(typeOf[Deserializer[_]], List(tpe)), withMacrosDisabled = true) match {
        case Some(tree) => q"private[this] val $name: ${tree.tpe} = $tree"
        case None       => q"private[this] lazy val $name: serializer.Deserializer[$tpe] = implicitly[serializer.Deserializer[$tpe]]"
      }
  ...
  private case class FieldSerializationInfo(objTpe: Type, field: FieldImpl, sharedSerializers: Vector[(Type, TermName)]) {
    ...
    val serializerValDef: Option[ValDef] = {
      if (field.serializer != null) {
        //Some(q"lazy val $serializerTermName: serializer.Serializer[$tpe] = ${field.serializer}")
  ...
  private case class FieldDeserializationInfo(objTpe: Type, field: FieldImpl, sharedDeserializers: Seq[(Type, TermName)]) {
    ...
    val deserializerTermName: TermName = {
      if (null == field.deserializer) sharedDeserializers.find{ _._1 =:= field.tpe }.head._2 else TermName(ctx.freshName(s"deser_${number}_${name}"))
    }
    ...
    val deserializerValDef: Option[ValDef] = {
      if (field.deserializer != null) {
        //Some(q"lazy val $deserializerTermName: serializer.Deserializer[$tpe] = ${field.deserializer}")
    ...
    def hasUserDefinedDefaultValue: Boolean = null != field.defaultValue
  ...
  def makeFieldImpl(tree: Tree): FieldImpl = {
    val q"new serializer.Field(..$args)" = tree
    makeFieldImpl(args)
  }
  ...
  def extractFieldAnnotations(tpe: Type): Seq[FieldImpl] = {
    ...
    val fields: Vector[FieldImpl] = for {
      ...
      val missingFields: FieldImpl = {
        val n: String = if (null == spec.name || spec.name == "") name else null
        FieldImpl(name = n)
      }
  ...
  private def combineFieldImpl(all: Vector[FieldImpl]): Vector[FieldImpl] = {
    combineFieldImplUsing(combineFieldImplUsing(all, _.number, (num: Int) => num == -1), _.name, (name: String) => null == name)
  }
...
//  }
  ...
  def findEnumSerializer[T: WeakTypeTag]: Option[Expr[SimpleSerializer[T]]] = {
    ...
    val tree: Tree = if (tpe <:< typeOf[java.lang.Enum[_]]) {
      ...
      //q"serializer.Primitive.string.map[$tpe]((enum: $tpe) => enum.name(), (s: String) => $staticTpe.valueOf(s), null)"
      q"serializer.Primitive.int.map[$tpe]((enum: $tpe) => enum.ordinal(), (idx: Int) => $staticTpe.values()(idx), null)"
    } else null
  ...
  def findOptionSerializer[T: WeakTypeTag]: Option[Expr[Serializer[T]]] = {
    ...
    val tree: Tree = if (tpe <:< typeOf[Option[_]]) {
      ...
      q"serializer.OptionSerializer[$arg]().asInstanceOf[serializer.Serializer[$tpe]]"
    } else null
  ...
  def findOptionDeserializer[T: WeakTypeTag]: Option[Expr[Deserializer[T]]] = {
    ...
    val tree: Tree = if (tpe <:< typeOf[Option[_]]) {
      ...
      q"serializer.OptionDeserializer[$arg]()"
    } else null
  ...
  def findAnyValSerializer[T: WeakTypeTag]: Option[Expr[Serializer[T]]] = {
    ...
    val tree: Tree = if (tpe <:< typeOf[AnyVal]) {
      ...
      q"""
        implicit object $name extends serializer.Serializer[$tpe] {
          ..${serInfo.serializerDeclarations}

          def serializeRaw(output: serializer.RawOutput, obj: $tpe): Unit = ${field.serializerTermName}.serializeRaw(output, ${field.fieldAccessor})
          def serializeNested(output: serializer.NestedOutput, obj: $tpe): Unit = ${field.serializerTermName}.serializeNested(output, ${field.fieldAccessor})
          def serializeField(output: serializer.FieldOutput, number: Int, name: String, obj: $tpe): Unit = ${field.serializerTermName}.serializeField(output, number, name, ${field.fieldAccessor})
        }
  ...
  def findAnyValDeserializer[T: WeakTypeTag]: Option[Expr[Deserializer[T]]] = {
    ...
    val tree: Tree = if (tpe <:< typeOf[AnyVal]) {
      ...
      q"""
        implicit object $name extends serializer.Deserializer[$tpe] {
          ..${serInfo.deserializerDeclarations}
          def defaultValue: $tpe = make(${field.deserializerTermName}.defaultValue)
          def deserializeRaw(input: serializer.RawInput): $tpe = make(${field.deserializerTermName}.deserializeRaw(input))
          def deserializeNested(input: serializer.NestedInput): $tpe = make(${field.deserializerTermName}.deserializeNested(input))
  ...
  private def checkCollectionSerializer[T](tpe: Type, colType: Type, serializerType: Type, useTupleValueAsElem: Boolean): Option[Expr[Serializer[T]]] = {
    ...
    val tree: Tree = if (tpe <:< colType) {
      ...
      q"""
        implicit val $proxyName: serializer.SerializerProxy[$tpe] = new serializer.SerializerProxy[$tpe]
        val $name: serializer.Serializer[$tpe] = new ${serTpe}[$elem, $tpe]()
        $proxyName.self = $name
        $name
      """
    } else null
  ...
  def makeNormalCollectionDeserializer(tpe: Type, elemTpe: Type): Tree = {
    ...
    val tree: Tree =
      if (tpe <:< VectorTpe) {
        ...
        //println(s"DETECTED VECTOR - $tpe - new serializer.VectorDeserializer[$elemTpe, $tpe]()")
        ...
        q"new serializer.JavaCollectionDeserializer[$elemTpe, $tpe]($newTpe)"
      } else {
        null
      }
    ...
    q"""
      implicit val $proxyName: serializer.DeserializerProxy[$tpe] = new serializer.DeserializerProxy[$tpe]()
      val $name: serializer.Deserializer[$tpe] = $tree
      $proxyName.self = $name
      $name
    """
  }
  ...
  def makeStringMapCollectionDeserializer(tpe: Type, elemTpe: Type): Tree = {
    ...
    val tree: Tree = if (!isScala213 && hasImplicit(appliedType(CanBuildFromType, List(WildcardType, elemTpe, tpe)))) {
      ...
      //val isTrait: Boolean = tpe.typeSymbol.asClass.isTrait
      //val newTpe: Tree = if (isTrait) q"new java.util.ArrayList[$elemTpe]()" else q"new $tpe" 
      //q"implicit val $name: serializer.Deserializer[$tpe] = new serializer.JavaCollectionDeserializer[$elemTpe, $tpe]($newTpe); $name"
    } else {
      null
    }
...
//  }
  ...
  def makeSimpleObjectSerializer[T: WeakTypeTag](fields: Seq[FieldImpl]): Expr[SimpleObjectSerializer[T]] = {
    ...
    val tree: Tree = q"""
      ...
      implicit val $name: serializer.SimpleObjectSerializer[$tpe] = new serializer.SimpleObjectSerializer[$tpe]()($serName, $deserName)
      $name
    """
  ...
  def makeObjectSerializer[T: WeakTypeTag](fields: Seq[FieldImpl]): Expr[ObjectSerializer[T]] = {
    ...
    val tree: Tree = q"""
    implicit object $name extends serializer.ObjectSerializer[$tpe] {
      ..${serInfo.serializerDeclarations}
    
      final def serializeRaw(output: serializer.RawOutput, v: $tpe): Unit = output.writeRawObject(v){ writeFun }
      final def serializeNested(output: serializer.NestedOutput, v: $tpe): Unit = output.writeNestedObject(v){ writeFun }
      final def serializeField(output: serializer.FieldOutput, number: Int, name: String, v: $tpe): Unit = output.writeFieldObject(number, name, v){ writeFun }
    
      private[this] val writeFun: Function2[serializer.FieldOutput, $tpe, Unit] = new Function2[serializer.FieldOutput, $tpe, Unit] {
        def apply(output: serializer.FieldOutput, obj: $tpe): Unit = {
          ..${serInfo.writes}
        }
      }
    }
  ...
  def makeObjectDeserializer[T: WeakTypeTag](fields: Seq[FieldImpl]): Expr[ObjectDeserializer[T]] = {
    ...
    val tree: Tree = q"""
    implicit object $name extends serializer.ObjectDeserializer[$tpe] {
      ...
      final def deserializeRaw(input: serializer.RawInput): $tpe = input.readRawObject{ readFun }
      final def deserializeNested(input: serializer.NestedInput): $tpe = input.readNestedObject{ readFun }
      
      private[this] val readFun: Function1[serializer.FieldInput, $tpe] = new Function1[serializer.FieldInput, $tpe] {
        def apply(input: serializer.FieldInput): $tpe = {
          ..${deserInfo.isSetVars}
          ..${deserInfo.readVars}
  ...
  private def getCompanionSymbol(tpe: Type): Symbol = {
    ...
    def patchedCompanionRef(tpe: Type): Tree = {
      ...
      global.gen.mkAttributedRef(globalType.prefix, original.companion.orElse {
        ...
        var callSiteContext = ctx.asInstanceOf[scala.reflect.macros.contexts.Context].callsiteTyper.asInstanceOf[global.analyzer.Typer].context
        var res: Symbol = NoSymbol
        while (res == NoSymbol && callSiteContext.outer != callSiteContext) {
          ...
          else callSiteContext = callSiteContext.outer
        }
        res
      }).asInstanceOf[Tree]
    }
  ...
  def getAccessorForMethod(tpe: Type, name: TermName): Option[Tree] = {
    getNoArgsMethod(tpe, name).flatMap { sym: MethodSymbol =>
      val select: Tree = if (tpe.typeSymbol.isModuleClass) q"${getCompanionSymbol(tpe).companion}" else q"this"
  ...
  private def JavaBeanTransientType: Type = ctx.mirror.staticClass("java.beans.Transient").toType
  private def XmlTransientType     : Type = ctx.mirror.staticClass("javax.xml.bind.annotation.XmlTransient").toType
